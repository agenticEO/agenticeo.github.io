...
export default function Home() {
  const [activeTab, setActiveTab] = useState<"overview" | "schedule" | "speakers" | "cfp">("overview");
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] flex flex-col selection:bg-sky-100 selection:text-sky-900">
          ...
          <div className="max-w-3xl space-y-6">
            ...
            <div className="flex flex-wrap gap-6 text-sm text-slate-300 pt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Anticipated: 50–80 Participants</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-white border-b border-slate-200/80 py-4 sticky top-0 z-50 shadow-sm backdrop-blur-md bg-white/95">
        <div className="container max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          {/* Tabs Navigation */}
          <div className="flex border-b border-slate-100 md:border-none w-full md:w-auto overflow-x-auto scrollbar-none">
            {(["overview", "schedule", "speakers", "cfp"] as const).map((tab) => (
              <button
                key={tab}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
            ...
            <Button size="sm" className="bg-sky-700 hover:bg-sky-800 text-white font-medium" onClick={() => setActiveTab("cfp")}>
      ...
      <main className="container max-w-6xl py-12 flex-grow">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-between items-start gap-4">
                  ...
                  <span className="text-slate-900 text-right">Double-blind, 2 reviews/paper</span>
                </div>
              </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            {activeTab === "overview" && (
              <div className="space-y-10 animate-fade-in">
                ...
                {/* Why This Workshop? */}
                <section className="space-y-6 bg-slate-50 p-6 md:p-8 rounded-xl border border-slate-100">
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    {KEYNOTE_SPEAKERS.map((speaker, idx) => (
                      <Card key={idx} className="overflow-hidden border-slate-200 hover:border-sky-300/80 transition-all duration-300 bg-white">
                        <CardContent className="p-6">
                          <div className="flex flex-col md:flex-row md:items-start gap-6">
                            
                            {/* Avatar Placeholder */}
                            <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-serif font-semibold text-xl shrink-0 border border-slate-200">
        ...
        <section id="organizers" className="mt-24 pt-12 border-t border-slate-200 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ORGANIZERS.map((org, idx) => (
              <Card key={idx} className="border-slate-200 hover:border-slate-300 transition-all bg-white flex flex-col justify-between">
        ...
        <section className="mt-24 pt-12 border-t border-slate-200 space-y-8">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-serif font-semibold text-slate-900">
              Program Committee
            </h2>
            <p className="text-slate-500 text-sm font-light">
              Reviewers and technical advisors supporting paper selection and quality assurance.
            </p>
          </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-700 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 font-semibold">Affiliation</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {PROGRAM_COMMITTEE.map((member, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-600">{member.affiliation}</td>
                      <td className="px-6 py-4 text-slate-500 text-xs">{member.role}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <footer className="bg-slate-900 text-slate-400 py-12 border-t border-slate-800 mt-24">
        <div className="container max-w-6xl flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-6 text-xs">
            <a href="#overview" className="hover:text-white transition-colors" onClick={() => setActiveTab("overview")}>Overview</a>
            <a href="#schedule" className="hover:text-white transition-colors" onClick={() => setActiveTab("schedule")}>Schedule</a>
            <a href="#speakers" className="hover:text-white transition-colors" onClick={() => setActiveTab("speakers")}>Speakers</a>
            <a href="#cfp" className="hover:text-white transition-colors" onClick={() => setActiveTab("cfp")}>Call for Papers</a>
          </div>
